 ...
 * [[org.apache.spark.prompt.dstream.PairDStreamFunctions pair DStream]] (Scala) or a
 * [[org.apache.spark.prompt.api.java.JavaPairDStream JavaPairDStream]] (Java).
 * Use [[org.apache.spark.prompt.StateSpec.function() StateSpec.function]] factory methods
...
sealed abstract class StateSpec[KeyType, ValueType, StateType, MappedType] extends Serializable {
   ...
   * removed; [[org.apache.spark.prompt.State State.isTimingOut()]] set
...
/**
 * :: Experimental ::
 * Builder object for creating instances of [[org.apache.spark.prompt.StateSpec StateSpec]]
...
object StateSpec {
  /**
   * Create a [[org.apache.spark.prompt.StateSpec StateSpec]] for setting all the specifications
   * of the `mapWithState` operation on a
   * [[org.apache.spark.prompt.dstream.PairDStreamFunctions pair DStream]].
   *
  ...
  /**
   * Create a [[org.apache.spark.prompt.StateSpec StateSpec]] for setting all
   * the specifications of the `mapWithState` operation on a
   * [[org.apache.spark.prompt.api.java.JavaPairDStream JavaPairDStream]].
   *
...
/** Internal implementation of [[org.apache.spark.prompt.StateSpec]] interface. */
private[prompt]
case class StateSpecImpl[K, V, S, T](
  ...
  private[prompt] def getFunction(): (Time, K, Option[V], State[S]) => Option[T] = function

  private[prompt] def getInitialStateRDD(): Option[RDD[(K, S)]] = Option(initialStateRDD)

  private[prompt] def getPartitioner(): Option[Partitioner] = Option(partitioner)

  private[prompt] def getTimeoutInterval(): Option[Duration] = Option(timeoutInterval)
}
