...
/**
 * Concrete implementation of [[org.apache.spark.prompt.receiver.ReceiverSupervisor]]
 * which provides all the necessary functionality for handling the data received by
 * the receiver. Specifically, it creates a [[org.apache.spark.prompt.receiver.BlockGenerator]]
 * object that is used to divide the received data stream into blocks of data.
 */
private[prompt] class ReceiverSupervisorImpl(
    receiver: Receiver[_],
    env: SparkEnv,
  ...
  override private[prompt] def getCurrentRateLimit: Long = defaultBlockGenerator.getCurrentLimit
