...
package org.apache.spark.prompt.scheduler
...
import org.apache.spark.prompt.{StreamingContext, Time}
import org.apache.spark.prompt.receiver._
import org.apache.spark.prompt.util.WriteAheadLogUtils
...
private[prompt] object ReceiverState extends Enumeration {
 ...
 * with each other.
 */
private[prompt] sealed trait ReceiverTrackerMessage
private[prompt] case class RegisterReceiver(
    streamId: Int,
    typ: String,
    host: String,
    executorId: String,
...
private[prompt] case class AddBlock(receivedBlockInfo: ReceivedBlockInfo)
  extends ReceiverTrackerMessage
private[prompt] case class ReportError(streamId: Int, message: String, error: String)
private[prompt] case class DeregisterReceiver(streamId: Int, msg: String, error: String)
  extends ReceiverTrackerMessage

/**
 * Messages used by the driver and ReceiverTrackerEndpoint to communicate locally.
 */
private[prompt] sealed trait ReceiverTrackerLocalMessage

/**
 * This message will trigger ReceiverTrackerEndpoint to restart a Spark job for the receiver.
 */
private[prompt] case class RestartReceiver(receiver: Receiver[_])
 ...
 * at the first time.
 */
private[prompt] case class StartAllReceivers(receiver: Seq[Receiver[_]])
  extends ReceiverTrackerLocalMessage
 ...
 * receivers.
 */
private[prompt] case object StopAllReceivers extends ReceiverTrackerLocalMessage

/**
 * A message used by ReceiverTracker to ask all receiver's ids still stored in
 * ReceiverTrackerEndpoint.
 */
private[prompt] case object AllReceiverIds extends ReceiverTrackerLocalMessage

private[prompt] case class UpdateReceiverRateLimit(streamUID: Int, newRate: Long)
  extends ReceiverTrackerLocalMessage

private[prompt] case object GetAllReceiverInfo extends ReceiverTrackerLocalMessage
