 ...
 * etc.) using a [[org.apache.spark.prompt.StreamingContext]] or it can be generated by
 ...
 * `window`. In addition, [[org.apache.spark.prompt.dstream.PairDStreamFunctions]] contains
...
abstract class DStream[T: ClassTag] (
    @transient private[prompt] var ssc: StreamingContext
  ...
  @transient
  private[prompt] var generatedRDDs = new HashMap[Time, RDD[T]]()

  // Time zero for the DStream
  private[prompt] var zeroTime: Time = null

  // Duration for which the DStream will remember each RDD created
  private[prompt] var rememberDuration: Duration = null

  // Storage level of the RDDs in the stream
  private[prompt] var storageLevel: StorageLevel = StorageLevel.NONE

  // Checkpoint details
  private[prompt] val mustCheckpoint = false
  private[prompt] var checkpointDuration: Duration = null
  private[prompt] val checkpointData = new DStreamCheckpointData(this)
  @transient
  private var restoredFromCheckpointData = false

  // Reference to whole DStream graph
  private[prompt] var graph: DStreamGraph = null

  private[prompt] def isInitialized = zeroTime != null

  // Duration for which the DStream requires its parent DStream to remember each RDD created
  private[prompt] def parentRememberDuration = rememberDuration

  /** Return the StreamingContext associated with this DStream */
  def context: StreamingContext = ssc

  /* Set the creation call site */
  private[prompt] val creationSite = DStream.getCreationSite()
   ...
   */
  protected[prompt] val baseScope: Option[String] = {
   ...
   * its parent DStreams.
   */
  private[prompt] def initialize(time: Time) {
  ...
  private[prompt] def validateAtStart() {
  ...
  private[prompt] def setContext(s: StreamingContext) {
    if (ssc != null && ssc != s) {
  ...
  private[prompt] def setGraph(g: DStreamGraph) {
  ...
  private[prompt] def remember(duration: Duration) {
  ...
  private[prompt] def isTimeValid(time: Time): Boolean = {
    if (!isInitialized) {
   ...
   */
  private[prompt] final def getOrCompute(time: Time): Option[RDD[T]] = {
   ...
   */
  protected[prompt] def createRDDWithLocalProperties[U](
      time: Time,
   ...
   */
  private[prompt] def generateJob(time: Time): Option[Job] = {
   ...
   */
  private[prompt] def clearMetadata(time: Time) {
   ...
   */
  private[prompt] def updateCheckpointData(currentTime: Time) {
  ...
  private[prompt] def clearCheckpointData(time: Time) {
   ...
   */
  private[prompt] def restoreCheckpointData() {
   ...
   */
  private[prompt] def register(): DStream[T] = {
...
object DStream {
  ...
  private[prompt] def getCreationSite(): CallSite = {
