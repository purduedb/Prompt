 ...
 * It implements Early Batch Release technique.
 * It provides all the necessary interfaces for handling the data received by the receiver.
 */
private[prompt] abstract class ReceiverSupervisor(
    receiver: Receiver[_],
    conf: SparkConf
  ) extends Logging {
  ...
  private[prompt] def getCurrentRateLimit: Long = Long.MaxValue

  /** Exception associated with the stopping of the receiver */
  @volatile protected var stoppingError: Throwable = null

  /** State of the receiver */
  @volatile private[prompt] var receiverState = Initialized
  ...
  /** Restart receiver with delay to account for EarlyBatchRelease */
