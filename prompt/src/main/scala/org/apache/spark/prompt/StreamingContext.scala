...
package org.apache.spark.prompt
...
import org.apache.spark.prompt.StreamingContextState._
import org.apache.spark.prompt.dstream._
import org.apache.spark.prompt.receiver.Receiver
import org.apache.spark.prompt.scheduler.{ExecutorAllocationManager, JobScheduler, StreamingListener}
import org.apache.spark.prompt.ui.{StreamingJobProgressListener, StreamingTab}
import org.apache.spark.util.{CallSite, ShutdownHookManager, ThreadUtils, Utils}

/**
 * Main entry point for Spark Streaming functionality. It provides methods used to create
 * [[org.apache.spark.prompt.dstream.DStream]]s from various input sources. It can be either
 ...
 */
class StreamingContext private[prompt](
    _sc: SparkContext,
    _cp: Checkpoint,
    _batchDur: Duration
  ) extends Logging {
  ...
  private[prompt] val isCheckpointPresent: Boolean = _cp != null

  private[prompt] val sc: SparkContext = {
    if (_sc != null) {
      _sc
  ...
  private[prompt] val conf = sc.conf

  private[prompt] val env = sc.env

  private[prompt] val graph: DStreamGraph = {
  ...
  private[prompt] var checkpointDir: String = {
    if (isCheckpointPresent) {
  ...
  private[prompt] val checkpointDuration: Duration = {
    if (isCheckpointPresent) _cp.checkpointDuration else graph.batchDuration
  }

  private[prompt] val scheduler = new JobScheduler(this)

  private[prompt] val waiter = new ContextWaiter

  private[prompt] val progressListener = new StreamingJobProgressListener(this)

  private[prompt] val uiTab: Option[StreamingTab] =
  ...
  private[prompt] val savedProperties = new AtomicReference[Properties](new Properties)

  private[prompt] def getStartSite(): CallSite = startSite.get()
  ...
  private[prompt] def isCheckpointingEnabled: Boolean = {
    checkpointDir != null
  }

  private[prompt] def initialCheckpoint: Checkpoint = {
    if (isCheckpointPresent) _cp else null
  }

  private[prompt] def getNewInputStreamId() = nextInputStreamId.getAndIncrement()
   ...
   */
  private[prompt] def withScope[U](body: => U): U = sparkContext.withScope(body)
   ...
   * Note: Return statements are NOT allowed in the given body.
   */
  private[prompt] def withNamedScope[U](name: String)(body: => U): U = {
  ...
  /**
   * Add a [[org.apache.spark.prompt.scheduler.StreamingListener]] object for
...
object StreamingContext extends Logging {
  ...
  private[prompt] def createNewSparkContext(conf: SparkConf): SparkContext = {
    new SparkContext(conf)
  }

  private[prompt] def createNewSparkContext(
      master: String,
      appName: String,
      sparkHome: String,
      jars: Seq[String],
  ...
  private[prompt] def rddToFileName[T](prefix: String, suffix: String, time: Time): String = {
