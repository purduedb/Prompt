...
import org.apache.spark.prompt.StreamingContext.rddToFileName
...
class PairDStreamFunctions[K, V](self: DStream[(K, V)])
  ...
  private[prompt] def ssc = self.ssc

  private[prompt] def sparkContext = self.context.sparkContext

  private[prompt] def defaultPartitioner(numPartitions: Int = self.ssc.sc.defaultParallelism) = {
