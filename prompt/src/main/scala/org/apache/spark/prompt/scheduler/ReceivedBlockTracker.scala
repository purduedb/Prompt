...
import org.apache.spark.prompt.util.{WriteAheadLog, WriteAheadLogUtils}
import org.apache.spark.util.{Clock, Utils}

/** Trait representing any event in the ReceivedBlockTracker that updates its state. */
private[prompt] sealed trait ReceivedBlockTrackerLogEvent

private[prompt] case class BlockAdditionEvent(receivedBlockInfo: ReceivedBlockInfo)
  extends ReceivedBlockTrackerLogEvent
private[prompt] case class BatchAllocationEvent(time: Time, allocatedBlocks: AllocatedBlocks)
  extends ReceivedBlockTrackerLogEvent
private[prompt] case class BatchCleanupEvent(times: Seq[Time])
  extends ReceivedBlockTrackerLogEvent
 ...
 */
private[prompt] class ReceivedBlockTracker(
    conf: SparkConf,
  ...
  private[prompt] def isWriteAheadLogEnabled: Boolean = writeAheadLogOption.nonEmpty
}

private[prompt] object ReceivedBlockTracker {
